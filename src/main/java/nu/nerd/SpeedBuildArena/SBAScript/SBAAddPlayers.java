                ...
                for (SBAPlot plot : plots) {
                    ...
                    if (rplot.contains(pos)) {
                        if (config.ADD_OWNERS) {
                            context.dispatchCommand(String.format("region addowner %s %s -w %s",
                                                                  rplot.getId(), player.getName(),
                                                                  worldName));
                            // rplot.getOwners().addPlayer(wgp.wrapPlayer(player));
                            // ^this does not update WG player cache in WG 6.
                        } else {
                            context.dispatchCommand(String.format("region addmember %s %s -w %s",
                                                                  rplot.getId(), player.getName(),
                                                                  worldName));
